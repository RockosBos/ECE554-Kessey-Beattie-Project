...
void PrintTemps (uint32_t TempC, uint32_t millisecond, uint32_t second, uint32_t minute);
...
while(1)
  {
        ...
        PrintTemps(ui32TempValueC, millisecond, second, minute);
...
void PrintTemps (uint32_t ui32TempValueC, uint32_t millisecond, uint32_t second, uint32_t minute)
{
    SysTickIntDisable();
    UARTprintf("%2d:%2d:%2d     %3d*C\n", minute, second, millisecond,ui32TempValueC);
    SysTickIntEnable();
